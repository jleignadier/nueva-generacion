...
-- Add qr_active_token column to events table
ALTER TABLE public.events ADD COLUMN qr_active_token TEXT NULL;

-- Replace award_event_points RPC to add token validation for qr_scan
...
BEGIN
  ...
  ELSIF p_check_in_method = 'qr_scan' THEN
    ...
    -- Validate QR token matches the active token on the event
    SELECT qr_active_token INTO v_active_token
    FROM public.events
    WHERE id = p_event_id;

    IF v_active_token IS NULL THEN
      RAISE EXCEPTION 'QR attendance is not active for this event';
    END IF;

    IF p_qr_token IS NULL OR p_qr_token != v_active_token THEN
